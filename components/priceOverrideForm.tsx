...
import { addPriceOverride, type PricingState } from "@/app/actions/pricing";
    ...
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-header/70">{t("overrideDate")}</span>
          <input type="date" name="date" required className={inputClass} />
        </label>
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-header/70">{t("overridePrice")}</span>
          <input
            type="number"
            name="price"
            min={0}
      ...
      <Button type="submit" disabled={pending} className="self-start px-5 py-2">
        {pending ? t("submitting") : t("addOverride")}
      </Button>
    </form>
  );
}
